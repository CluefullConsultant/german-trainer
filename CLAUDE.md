...
| `sentence_correction.py` | Powers **Korrektur** (new 16.09.2026) — free-form German text box (informal/incomplete register explicitly allowed, not "corrected away"), returns a corrected version plus a per-mistake list (original → correction, one-line reason), streamed live via `client.messages.stream` so it types out instead of a blank spinner. **Runs on Sonnet, not Haiku** — briefly tried Haiku for ~2x speed, but user explicitly prioritized accuracy over speed after Haiku produced a no-op "correction" (identical original/correction text) in testing; streaming was kept since it's a pure UX win independent of model choice, only the model reverted. Standalone, no DB write — not wired into the `db.save_claude_feedback`/`get_top_errors` mistake-tracking system since that needs an exercise/submission row per the current schema; a good hook point for roadmap step 7 (feedback rebuild) rather than something to force in now. |
